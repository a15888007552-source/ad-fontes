#!/usr/bin/env python3
"""Browser/network smoke test for the Qin-Han externalized-media runtime.

The test exercises the module's runtime surface in a real Chromium browser:
every media request must resolve through the current R2 public host, the
retired workers.dev host must never be contacted, and no externalized media
may be requested from the local module tree.  Provenance trail images are
scrolled into view individually because they lazy-load.
"""
...
MODULE = "qinhan"
PAGE_URL = "http://127.0.0.1:8765/modules/qinhan/"
MEDIA_HOST = "pub-2f296678a1134f0fa45cf651ddd6f956.r2.dev"
RETIRED_WORKER_HOST = "ad-fontes-media.gusgumee777.workers.dev"
FROZEN_MANIFEST_PATH = Path("data/qinhan-externalized-media.json")
PROVENANCE_EXPECTED = 4
MEDIA_SUFFIXES = {
    ".jpg",
    ".jpeg",
    ".png",
...
def load_frozen_paths(repo_root: Path) -> set[str]:
    ...
    return {str(item["path"]) for item in manifest["objects"]}
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("artifacts/qinhan-browser-smoke.json"))
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            if tracker.local_module_requests:
                fail(failures, "local module media request observed")
            ...
            try:
                ...
                page.screenshot(path=str(args.output.parent / "qinhan-home.png"), full_page=True)
    ...
    print(f"QINHAN_BROWSER_SMOKE={report['status']}")
    ...
    print(f"LOCAL_MODULE_MEDIA={report.get('localModuleMediaRequests', 0)}")
