(() => {
  ...
  searchOptions.innerHTML = atlas.types.map((type) => `<option value="${mono(type.nameZh)}">${mono(type.romanization)} · ${mono(type.nameEn)}</option>`).join('');
  ...
  function findType(value) {
    ...
    return atlas.types.find((type) => [type.id, type.slug, type.nameZh, type.nameEn, type.romanization].some((field) => String(field || '').toLowerCase().replace(/[\s·/_-]/g, '') === needle))
      || atlas.types.find((type) => [type.nameZh, type.nameEn, type.romanization].some((field) => String(field || '').toLowerCase().replace(/[\s·/_-]/g, '').includes(needle)));
  }
