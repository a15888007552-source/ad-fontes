...
function syncItemToUrl(id, { historyMode = "push" } = {}) {
  ...
  if (url.hash.startsWith("#artifact-")) {
    try {
      const legacyId = decodeURIComponent(url.hash.slice(1));
      legacyArtifactHash = Boolean(artifactById(legacyId));
    } catch {
...
function openArtifactById(
  ...
  const dialogAlreadyOpen =
    elements.artifactDialog.open || elements.artifactDialog.hasAttribute("open");
...
function bindEvents() {
  ...
  elements.artifactDialog.addEventListener("click", (event) => {
    if (event.target === elements.artifactDialog) {
      closeArtifact({
        syncUrl: true,
        restoreFocus: true,
      });
  ...
  elements.artifactDialog.addEventListener("close", () => {
    ...
    state.currentPhotos = [];
    if (restoreFocusAfterDialogClose && lastFocusedElement?.isConnected) {
